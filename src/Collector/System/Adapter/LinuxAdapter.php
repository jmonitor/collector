...
class LinuxAdapter implements AdapterInterface
{
    ...
    {
        if (!array_key_exists('timezone', $this->propertyCache)) {
            $this->propertyCache['timezone'] = $this->doGetTimeZone();
        }

        return $this->propertyCache['timezone'];
    }
    ...
    private function getTimeZoneFromLocalTime(): ?string
    {
        ...
        if (strpos($localTime, 'zoneinfo/') !== false) {
