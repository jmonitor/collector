...
- **Collectors**: All collectors must implement `Jmonitor\Collector\CollectorInterface`. Optionally implement `BootableCollectorInterface` (for initialization) and/or `ResetInterface` (for state clearing). If a collector implements `LoggerAwareInterface`, the logger is injected automatically during boot.
